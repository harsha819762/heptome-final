...
import { 
  ...
  IoSparklesOutline, IoHeartOutline, IoChevronForwardOutline 
...
export default function ServiceDetail() {
  ...
  const { setBookingField, setBookingStep } = useBooking();
  ...
  return (
    ...
    >
      ...
      <nav className="text-xs font-semibold text-slate-400 flex items-center gap-1.5 px-4 sm:px-0">
        ...
        <Link to={`/services/${category.id}`} className="hover:text-slate-600">{category.category}</Link>
        <IoChevronForwardOutline />
